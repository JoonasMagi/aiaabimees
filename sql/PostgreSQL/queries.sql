-- PostgreSQL mõtestatud SELECT päringud

-- 1. Kasutajate taimede arv ja keskmine kasvatusaeg
SELECT 
    u.username,
    COUNT(up.user_plant_id) as taimede_arv,
    ROUND(AVG(up.est_cropping), 2) as keskmine_kasvatusaeg_paevades
FROM users u
...
SELECT 
    ...
    ROUND(AVG(up.est_cropping), 2) as keskmine_kasvatusaeg
FROM all_plants ap
...
SELECT 
    ...
    STRING_AGG(DISTINCT u.username, ', ') as kasutajad
FROM soil_types st
...
SELECT 
    EXTRACT(MONTH FROM up.planting_time) as kuu,
    TO_CHAR(up.planting_time, 'Month') as kuu_nimi,
    COUNT(*) as istutatud_taimede_arv,
    ROUND(AVG(up.est_cropping), 2) as keskmine_kasvatusaeg
FROM user_plants up
WHERE up.is_deleted = FALSE
GROUP BY EXTRACT(MONTH FROM up.planting_time), TO_CHAR(up.planting_time, 'Month')
ORDER BY kuu;

-- 8. Temperatuuri ja niiskuse korrelatsioon (PostgreSQL spetsiifiline)
SELECT 
    location,
    ROUND(CORR(temperature, humidity), 4) as temp_niiskus_korrelatsioon,
    COUNT(*) as andmepunktide_arv
FROM weather_data
GROUP BY location
HAVING COUNT(*) > 3
ORDER BY temp_niiskus_korrelatsioon DESC;
